...
for(const source of [schema,storage,backup,dataManagementStore])assert.ok(source.includes('financialEvents'),'Finanzereignisse müssen durch Schema, Persistenz, Backup und Reset geführt werden');
...
assert.ok(dataManagementStore.includes('s.financialEvents=[]'));assert.ok(backup.includes('S.financialEvents'));
