...
const [index,css,shellEvents,files]=await Promise.all([
  read('index.html'),read('css/app.css'),read('js/app-shell-events.js'),readdir(new URL('../css/',import.meta.url))
]);
...
assert.deepEqual(files.filter(file=>file.endsWith('.css')).sort(),['app.css'],'css/app.css muss die einzige CSS-Datei im Repository sein');
...
console.log('Phase-D-UI-Vertrag erfolgreich geprüft: eine CSS-Quelle, ein Spacing-Vertrag.');
