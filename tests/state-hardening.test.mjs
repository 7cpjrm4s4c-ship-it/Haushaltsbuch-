...

{
  ...
  const normalized=context.StateSchema.normalize({data:[],cats:[{id:'c1'}],kredite:null,years:['2028','x',2026,2028,1999,2201],buchungen:'invalid',budgets:[],recurringRules:[{id:'r1'}],amountAdjustments:[{id:'a1'}],oneTimeEntries:[{id:'o1'}],forecastAssets:{cash:1200,etf:'500',other:-2},forecastAssumptions:{annualReturns:{etf:7,callMoney:'2.5',other:-150},purchasingPowerInflation:2.2,savingsTarget:'etf'}},{defaultYears:()=>[2026,2027,2028]});
  assert.equal(normalized.schemaVersion,3);assert.deepEqual([...normalized.years],[2026,2028]);assert.equal(normalized.cats.length,1);assert.equal(normalized.kredite.length,0);assert.equal(normalized.buchungen.length,0);
  assert.equal(normalized.amountAdjustments.length,1);assert.equal(normalized.oneTimeEntries.length,1);assert.equal(normalized.forecastAssets.cash,1200);assert.equal(normalized.forecastAssets.etf,500);assert.equal(normalized.forecastAssets.other,0);
...

{
  ...
  assert.equal(migrated.schemaVersion,3);assert.deepEqual([...S.years],[2026,2027]);assert.equal(S.kredite.length,0);assert.deepEqual([...S.amountAdjustments],[]);assert.deepEqual([...S.oneTimeEntries],[]);assert.equal(S.forecastAssets.cash,0);assert.equal(S.forecastAssumptions.savingsTarget,'etf');assert.equal(S.forecastAssumptions.purchasingPowerInflation,2);
}
