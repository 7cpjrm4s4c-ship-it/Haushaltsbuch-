...

{
  ...
  assert.equal(normalized.forecastAssumptions.annualReturns.etf,7);assert.equal(normalized.forecastAssumptions.annualReturns.callMoney,2.5);assert.equal(normalized.forecastAssumptions.annualReturns.other,-99);assert.equal(normalized.forecastAssumptions.purchasingPowerInflation,2.2);assert.equal(normalized.forecastAssumptions.savingsTarget,'etf');assert.equal(normalized.forecastAccounts.length,2);assert.equal(normalized.forecastAccounts.find(item=>item.name==='ETF').annualReturn,7);
}
