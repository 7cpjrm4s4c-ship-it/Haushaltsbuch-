...
// Planungslogik + Dashboard-Monatsberechnung
{
  ...
  const context=makeContext({S,gv:()=>0,calcMonth:()=>({}),dkey:(y,m,id)=>`${y}_${m}_${id}`,persist:()=>{},closeGenSheet:()=>{},render:()=>{},toast:()=>{},uid:()=> 'test-id',document:{getElementById:()=>null}});
  await run('js/app-extension-registry.js',context);await run('js/booking-store.js',context);await run('js/planning-events.js',context);await run('js/data-consistency.js',context);await run('js/app-extension-bindings.js',context);
