...
const [engine,adapter,renderers,viewModel,view,dataConsistency,bookingStore,dashboard]=await Promise.all([
  read('js/forecast-engine.js'),read('js/forecast-adapter.js'),read('js/forecast-view-renderers.js'),read('js/forecast-view-model.js'),read('js/forecast-view.js'),read('js/data-consistency.js'),read('js/booking-store.js'),read('js/dashboard-view.js')
]);
...
assert.ok(adapter.includes('BookingStore.inflowsByMonth()'),'Prognosen müssen einmalige Zahlungseingänge vor der Monatsschleife indizieren');
