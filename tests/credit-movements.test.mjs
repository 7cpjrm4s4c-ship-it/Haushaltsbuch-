...
assert.equal(context.CreditMovementStore.displayEntries(2026,0).filter(item=>item.type==='interest').length,1);
...
const month=context.consistentMonthCalculation(2026,0);assert.ok(Math.abs(month.v+398.84)<1e-9);assert.ok(Math.abs(month.saldo-398.84)<1e-9,'Kreditabrufe und -zahlungen müssen spiegelbildlich auf das Hauptkonto wirken');
...
const markup=context.CreditUi.view();assert.match(markup,/Rahmenkredit/);assert.match(markup,/ACT\/365/);assert.match(markup,/Sondertilgung erfassen/);assert.match(markup,/Bewegung erfassen/);
