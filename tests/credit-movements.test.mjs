...
assert.equal(januaryWithRate.scheduledPayment,51.16);assert.equal(januaryWithRate.scheduledPrincipal,50);assert.equal(januaryWithRate.closingBalance,550);
...
context.S.kredite[0]={...frame,m:200};const principalMonth=context.consistentMonthCalculation(2026,0);assert.equal(principalMonth.k,201.16,'200 Euro Tilgung und 1,16 Euro Zinsen müssen das Hauptkonto mit 201,16 Euro belasten');assert.equal(context.creditBalanceAt(context.S.kredite[0],2026,1,context.S.creditMovements),400,'Die Folgerestschuld muss trotz zusätzlicher Zinszahlung um volle 200 Euro sinken');context.S.kredite[0]=frame;
...
context.S.kredite[0]=frameWithRate;const rateForecast=context.forecastCreditSchedule(2026,0,2026,0).get('2026-0');assert.equal(rateForecast.creditPayments,551.16);assert.ok(Math.abs(rateForecast.debt-1450)<1e-9);context.S.kredite[0]=frame;
...
const markup=context.CreditUi.view();assert.match(markup,/Rahmenkredit/);assert.match(markup,/ACT\/365/);assert.match(markup,/Monatliche Tilgung/);assert.match(markup,/Zinsen zusätzlich/);assert.match(markup,/Gesamtzahlung/);assert.match(markup,/Sondertilgung erfassen/);assert.match(markup,/Bewegung erfassen/);
let creditSheet='';context.openGenSheet=value=>{creditSheet=value;};context.CreditUi.openEdit('frame-1');assert.match(creditSheet,/Monatliche Tilgung/);assert.match(creditSheet,/Zinsen werden zusätzlich/);assert.match(creditSheet,/id="kframepayment"/);
