...
(function(root){
  ...
  function evaluateGoal(rawGoal,forecast){
    const baseYear=forecast?.months?.[0]?.year||new Date().getFullYear(),goal=normalizeGoal(rawGoal,baseYear),allMonths=Array.isArray(forecast?.months)?forecast.months:[],months=monthsUntilTarget(allMonths,goal),startNetWorth=Number(forecast?.summary?.startAssets||0)-Number(forecast?.summary?.startDebt||0);
    ...
    if(goal.type==='minLiquidity'){
      const critical=months.reduce((min,row)=>Number(row.liquidity||0)<Number(min.liquidity||0)?row:min,months[0]),achieved=Number(critical.liquidity||0)>=goal.targetAmount;
      return {goal,status:achieved?'achieved':'missed',achieved,firstAchieved:achieved?{year:months[0].year,month:months[0].month}:null,value:round2(critical.liquidity),gap:round2(Math.max(0,goal.targetAmount-Number(critical.liquidity||0))),critical:{year:critical.year,month:critical.month,value:round2(critical.liquidity)}};
    }
    const predicate=goal.type==='debtFree'?value=>value<=0.005:value=>value>=goal.targetAmount;
    const first=months.find(row=>predicate(metricValue(row,goal.type,startNetWorth)))||null,last=months[months.length-1],value=metricValue(last,goal.type,startNetWorth),achieved=Boolean(first),gap=goal.type==='debtFree'?Math.max(0,value):Math.max(0,goal.targetAmount-value);
    return {goal,status:achieved?'achieved':'missed',achieved,firstAchieved:first?{year:first.year,month:first.month}:null,value:round2(value),gap:round2(gap),critical:null};
  }
