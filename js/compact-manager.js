...
function individualSavingsTransferGroup(year,month){
  ...
  const items=SavingsStore.transfers().filter(item=>item.direction==='deposit'&&Number(item.year)===Number(year)&&Number(item.month)===Number(month));
  if(!items.length)return '';
  const total=items.reduce((sum,item)=>sum+Number(item.amount||0),0);
  return `<details class="manager-group"><summary><div class="manager-group-title">Sparanlagen</div><div class="manager-group-meta">${items.length} · <span class="manager-total">${fmt(total)}</span></div><span class="manager-chevron">▼</span></summary><div class="manager-group-body">${items.map(item=>{const account=accounts.get(item.accountId);return `<details class="manager-entry"><summary><div class="manager-entry-main"><div class="manager-entry-title">${esc(item.note||account?.name||'Einzahlung')}</div><div class="manager-entry-sub">Einzeltransfer · ${esc(account?.name||'Sparanlage')} · ${MF[item.month]} ${item.year}</div></div><div class="manager-entry-value is-expense">-${fmt(item.amount)}</div><span class="manager-chevron">▼</span></summary><div class="manager-entry-actions">${managerButton('Sparanlage öffnen',`nav('savings')`)}${managerButton('Löschen',`deleteSavingsTransfer('${esc(item.id)}')`,true)}</div></details>`;}).join('')}</div></details>`;
}
