...
function vDashboard(){
  ...
  return `<div class="desktop-page-title">Dashboard</div><div class="layout-grid dashboard-grid"><div class="grid-primary">
    ...
    <div class="month-grid" id="monthGrid">${chips}</div><div class="hero"><div class="hero-bg"></div><div class="hero-content"><div class="hero-lbl">Netto-Saldo · ${MF[mo]} ${y}</div><div class="hero-val ${m.saldo>=0?'pos':'neg'}">${fmtS(m.saldo)}</div><div class="hero-sub hero-sub-four"><div class="hsi"><div class="hsi-lbl">Einnahmen</div><div class="hsi-val">${fmtS(m.e)}</div></div><div class="hsi"><div class="hsi-lbl">Ausgaben</div><div class="hsi-val">${fmtS(m.aus)}</div></div><div class="hsi"><div class="hsi-lbl">Kontostand</div><div class="hsi-val">${balance===null?'—':fmtS(balance)}</div></div><div class="hsi"><div class="hsi-lbl">Zeitraum</div><div class="hsi-val">${MS[mo]} ${y}</div></div></div></div></div>
