...
(() => {
  ...
  const downloadJson = payload => {
    ...
    link.href = url;
    link.download = `Haushaltsbuch_Backup_${stamp}.json`;
  ...
  window.createHouseholdBackup = function createHouseholdBackup() {
    const payload = snapshot();
    downloadJson(payload);
    dirty = false;
    writeMeta({ lastBackupAt: payload.createdAt, changesSinceBackup: 0 });
  ...
  function validBackup(payload) {
    ...
    return d && typeof d === 'object' && Array.isArray(d.cats) && Array.isArray(d.kredite);
  }
    ...
    if (!input) {
      ...
      input.addEventListener('change', async event => {
        ...
        try {
          ...
          if (!validBackup(payload)) throw new Error('invalid');
          pendingBackup = payload;
          openGenSheet(previewMarkup(payload));
        } catch (_) {
          toast('Die Datei ist kein gültiges Haushaltsbuch-Backup', 'err');
        }
      });
  ...
  window.applyHouseholdBackup = function applyHouseholdBackup(mode) {
    if (!pendingBackup) return;
    const d = pendingBackup.appData || pendingBackup;
    if (mode === 'replace') {
      S.data = d.data || {};
      S.cats = d.cats || [];
      S.kredite = d.kredite || [];
