...
(() => {
  ...
  function previewMarkup(payload){
    ...
    return `<div class="sheet-title">Backup prüfen</div><div class="backup-preview"><div><span>Erstellt</span><strong>${esc(dateText(created))}</strong></div><div><span>Kategorien</span><strong>${d.cats?.length||0}</strong></div><div><span>Kredite</span><strong>${d.kredite?.length||0}</strong></div><div><span>Sparanlagen</span><strong>${d.savingsAccounts?.length||0}</strong></div><div><span>Buchungen</span><strong>${d.buchungen?.length||0}</strong></div><div><span>Planungsregeln</span><strong>${rules}</strong></div><div><span>Finanzereignisse</span><strong>${d.financialEvents?.length||0}</strong></div><div><span>Prognoseszenarien</span><strong>${d.forecastScenarios?.length||0}</strong></div><div><span>Finanzziele</span><strong>${d.forecastGoals?.length||0}</strong></div><div><span>Prognose-Startvermögen</span><strong>${fmt(assetTotal(d))}</strong></div></div><p class="backup-note">„Ersetzen“ überschreibt die lokalen Daten. „Zusammenführen“ ergänzt Datensätze anhand ihrer ID.</p><div class="backup-actions"><button class="btn btn-primary" onclick="applyHouseholdBackup('replace')">Alle Daten ersetzen</button><button class="btn" onclick="applyHouseholdBackup('merge')">Zusammenführen</button><button class="btn" onclick="closeGenSheet()">Abbrechen</button></div>`;
  }
