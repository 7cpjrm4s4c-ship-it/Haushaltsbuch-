/* Kreditverwaltung: monatliche Restschuld, Zinsanteil und Tilgung. */
'use strict';
...
vKredite=function(){
  ...
  const cards=S.kredite.map(k=>{
    ...
    return `<div class="kredit-item">
      ...
      <div class="krow"><span>Restschuld ${MF[mo]} ${y}</span><span>${fmt(balance)}</span></div>
      <div class="krow"><span>Monatl. Rate</span><span>${fmt(Number(k.m||0))}</span></div>
      <div class="krow"><span>Davon Zinsen</span><span>${fmt(interest)}</span></div>
      <div class="krow"><span>Davon Tilgung</span><span>${fmt(principal)}</span></div>
      <div class="krow"><span>Zinssatz</span><span>${Number(k.z||0).toFixed(2)} %</span></div>
      <div class="krow"><span>Laufzeit bis</span><span>${esc(creditEndDateAt(k,y,mo))}</span></div>
  ...
  return `<div class="desktop-page-title">Kredite</div>
    <div class="hero"><div class="hero-bg"></div><div class="hero-content">
      <div class="hero-lbl">Gesamtschulden · ${MF[mo]} ${y}</div><div class="hero-val neg">${fmtS(tR)}</div>
      <div class="hero-sub">
...
kreditForm=function(k={}){
  ...
  return `<div class="field"><div class="lbl">Name</div><input class="inp" id="kn" value="${esc(k.n||'')}" placeholder="z. B. Deutsche Bank"/></div>
    ...
    <div class="field"><div class="lbl">Eingegebene Restschuld (€)</div><input class="inp" type="number" id="kr" value="${k.r??''}" min="0" step="0.01" inputmode="decimal" placeholder="0,00"/></div>
    <div class="form-grid two">
      <div class="field"><div class="lbl">Stand Monat</div><div class="sw"><select class="sel" id="krm">${MF.map((name,index)=>`<option value="${index}"${index===referenceMonth?' selected':''}>${name}</option>`).join('')}</select></div></div>
      <div class="field"><div class="lbl">Stand Jahr</div><div class="sw"><select class="sel" id="kry">${yearOptions}</select></div></div>
    </div>
    ...
    <div class="annual-note">Die eingegebene Restschuld gilt für den gewählten Stichtagsmonat. Für andere Monate wird sie automatisch mit Zinsen und Tilgung fortgeschrieben.</div>`;
};
