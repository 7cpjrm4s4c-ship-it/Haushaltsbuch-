...
(function(root,factory){
  ...
  function monthlyRate(annualPercent){
    const annual=clamp(annualPercent,-99,100,0)/100;
    return Math.pow(1+annual,1/12)-1;
  }

  function normalizeBaseMonth(item){
    return {year:Number(item?.year),month:Number(item?.month),income:Math.max(0,Number(item?.income)||0),fixed:Math.max(0,Number(item?.fixed)||0),savings:Math.max(0,Number(item?.savings)||0),creditPayments:Math.max(0,Number(item?.creditPayments)||0),debt:Math.max(0,Number(item?.debt)||0)};
  }
  ...
  function normalizeReturns(input){
    const source=input?.annualReturns&&typeof input.annualReturns==='object'?input.annualReturns:{};
    return Object.fromEntries(ASSET_FIELDS.map(field=>[field,clamp(source[field],-99,100,0)]));
  }
  ...
  function assetTotals(assets){
    const liquidity=sumFields(assets,LIQUID_FIELDS);
    const total=sumFields(assets,ASSET_FIELDS);
    return {liquidity,investments:total-liquidity,total};
  }

  function applyMonthlyReturns(assets,annualReturns){
    let earned=0;
    for(const field of ASSET_FIELDS){
      const balance=Number(assets[field]||0);
      if(balance<=0)continue;
      const gain=balance*monthlyRate(annualReturns[field]);
      assets[field]=balance+gain;earned+=gain;
    }
    return earned;
  }
  ...
  function project(input){
    ...
    for(let i=0;i<baseMonths.length;i++){
      ...
      const investmentReturn=applyMonthlyReturns(assets,annualReturns);
      cumulativeReturns+=investmentReturn;
      ...
      cumulative+=saldo;accumulatedSavings+=base.savings;
      assets.cash=Number(assets.cash||0)+saldo;
      assets[savingsTarget]=Number(assets[savingsTarget]||0)+base.savings;
      const totals=assetTotals(assets);
      const netWorth=totals.total-base.debt;
      const inflationFactor=Math.pow(1+realInflation,(i+1)/12);
    ...
    return {months,years,summary:{startAssets,startLiquidity:round2(startTotals.liquidity),startInvestments:round2(startTotals.investments),endAssets:last.assets,endLiquidity:last.liquidity,endInvestments:last.investments,endDebt:last.debt,endNetWorth:last.netWorth,endRealNetWorth:last.realNetWorth,cumulative:last.cumulative,cumulativeReturns:last.cumulativeReturns,minLiquidity:round2(minLiquidity),minLiquidityYear,minLiquidityMonth}};
  }

  function aggregateYears(months){
    const map=new Map();
    for(const item of months||[]){
      if(!map.has(item.year))map.set(item.year,{year:item.year,income:0,fixed:0,variable:0,creditPayments:0,savings:0,investmentReturn:0,expenses:0,saldo:0,endDebt:0,endCumulative:0,endLiquidity:0,endInvestments:0,endAssets:0,endNetWorth:0,endRealNetWorth:0});
      const row=map.get(item.year);row.income+=item.income;row.fixed+=item.fixed;row.variable+=item.variable;row.creditPayments+=item.creditPayments;row.savings+=item.savings;row.investmentReturn+=item.investmentReturn;row.expenses+=item.expenses;row.saldo+=item.saldo;
