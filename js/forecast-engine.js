/**
 * Reine Prognose-Engine ohne DOM- oder App-State-Abhängigkeiten.
...
(function(root,factory){
  ...
  const SCENARIOS=Object.freeze({optimistic:Object.freeze({key:'optimistic',label:'Optimistisch',variableFactor:0.95}),realistic:Object.freeze({key:'realistic',label:'Realistisch',variableFactor:1}),cautious:Object.freeze({key:'cautious',label:'Vorsichtig',variableFactor:1.10})});
  ...
  /**
   * Berechnet den historischen Durchschnitt variabler Ausgaben über den gewünschten Rückblick.
  ...
  function normalizeBaseMonth(item){return {year:Number(item?.year),month:Number(item?.month),income:Math.max(0,Number(item?.income)||0),fixed:Math.max(0,Number(item?.fixed)||0),savings:Math.max(0,Number(item?.savings)||0),savingsWithdrawals:Math.max(0,Number(item?.savingsWithdrawals)||0),accountTransfers:Array.isArray(item?.accountTransfers)?item.accountTransfers.map(entry=>({accountId:String(entry.accountId||''),deposits:Math.max(0,Number(entry.deposits)||0),withdrawals:Math.max(0,Number(entry.withdrawals)||0)})):[],creditPayments:Math.max(0,Number(item?.creditPayments)||0),creditDrawdowns:Math.max(0,Number(item?.creditDrawdowns)||0),creditInterest:Math.max(0,Number(item?.creditInterest)||0),specialRepayment:Math.max(0,Number(item?.specialRepayment)||0),openingDebt:Math.max(0,Number(item?.openingDebt)||0),debt:Math.max(0,Number(item?.debt)||0),variableReduction:Math.max(0,Number(item?.variableReduction)||0),financialEvents:Array.isArray(item?.financialEvents)?item.financialEvents.map(event=>({...event})):[]};}
  ...
  /**
   * Erstellt eine deterministische Monats- und Jahresprognose aus vollständig vorbereiteten Eingabedaten.
   * @param {Object} input Prognoseeingabe; `baseMonths` muss bereits app-spezifisch aufbereitet sein.
   * @returns {{months:Array<Object>,years:Array<Object>,summary:Object}} Projektion samt Monatswerten, Jahresaggregation und Zusammenfassung.
   * @throws {RangeError} Wenn der Prognosehorizont mehr als 601 Monate umfasst.
   */
  function project(input){
    ...
    for(let i=0;i<baseMonths.length;i++){
      ...
      const expenses=base.fixed+variable+base.creditPayments+base.specialRepayment+base.savings, saldo=base.income+base.savingsWithdrawals+base.creditDrawdowns-expenses;
      cumulative+=saldo;accumulatedSavings+=base.savings;totalSpecialRepayments+=base.specialRepayment;
      const cashAccount=liquidTarget(),managedDeposits=base.accountTransfers.reduce((sum,item)=>sum+item.deposits,0);if(cashAccount)cashAccount.balance+=saldo;for(const transfer of base.accountTransfers){const target=accounts.find(item=>item.id===`savings_${transfer.accountId}`);if(target)target.balance+=transfer.deposits-transfer.withdrawals;}const savingAccount=investmentTarget();if(savingAccount)savingAccount.balance+=Math.max(0,base.savings-managedDeposits);
  ...
  /** @param {Array<Object>} months Prognosemonate. @returns {Array<Object>} Aggregierte Jahreswerte. */
  function aggregateYears(months){const map=new Map();for(const item of months||[]){if(!map.has(item.year))map.set(item.year,{year:item.year,income:0,fixed:0,variable:0,creditPayments:0,creditDrawdowns:0,creditInterest:0,specialRepayment:0,savings:0,investmentReturn:0,expenses:0,saldo:0,endDebt:0,endCumulative:0,endLiquidity:0,endInvestments:0,endAssets:0,endNetWorth:0,endRealNetWorth:0,eventCount:0});const row=map.get(item.year);row.income+=item.income;row.fixed+=item.fixed;row.variable+=item.variable;row.creditPayments+=item.creditPayments;row.creditDrawdowns+=item.creditDrawdowns;row.creditInterest+=item.creditInterest;row.specialRepayment+=item.specialRepayment;row.savings+=item.savings;row.investmentReturn+=item.investmentReturn;row.expenses+=item.expenses;row.saldo+=item.saldo;row.eventCount+=item.financialEvents.length;row.endDebt=item.debt;row.endCumulative=item.cumulative;row.endLiquidity=item.liquidity;row.endInvestments=item.investments;row.endAssets=item.assets;row.endNetWorth=item.netWorth;row.endRealNetWorth=item.realNetWorth;}return [...map.values()].map(row=>Object.fromEntries(Object.entries(row).map(([key,value])=>[key,['year','eventCount'].includes(key)?value:round2(value)])));}
