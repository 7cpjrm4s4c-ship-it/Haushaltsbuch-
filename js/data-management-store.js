...
(function(root){
  ...
  function applyFactoryState(){const s=state(),date=currentDate();s.data={};s.cats=[...factoryFixedPositions(),...factoryVariableCategories()];s.kredite=factoryCredit();s.buchungen=factoryBookings();s.budgets={};s.recurringRules=[];s.annualAdjustments=[];s.percentageAdjustments=[];s.amountAdjustments=[];s.oneTimeEntries=[];s.accountBalances={};s.savingsAccounts=[];s.savingsTransfers=[];s.forecastAssets=emptyForecastAssets();s.forecastAssumptions=defaultForecastAssumptions();s.forecastAccounts=[];s.financialEvents=[];s.forecastScenarios=[];s.forecastGoals=[];s.years=years();s.year=date.getFullYear();s.month=date.getMonth();sortCategoriesInPlace();}
  function clearBookings(){const s=state();s.buchungen=[];s.budgets={};save();}
  function deleteAllEntries(){const s=state();s.data={};s.buchungen=[];s.budgets={};s.kredite=[];s.recurringRules=[];s.annualAdjustments=[];s.percentageAdjustments=[];s.amountAdjustments=[];s.oneTimeEntries=[];s.accountBalances={};s.savingsAccounts=[];s.savingsTransfers=[];s.financialEvents=[];s.forecastScenarios=[];s.forecastGoals=[];s.forecastAssets=emptyForecastAssets();s.forecastAssumptions=defaultForecastAssumptions();s.forecastAccounts=[];s.cats=factoryVariableCategories();sortCategoriesInPlace();save();}
