...
(function(root){
  ...
  function factoryBookings(){const date=currentDate(),y=date.getFullYear(),m=date.getMonth(),ts=Date.now();return [{id:'b_factory_1',direction:'expense',catId:'v_factory_6',bezeichnung:'Beispiel Kleidung',betrag:0,month:m,year:y,ts},{id:'b_factory_2',direction:'expense',catId:'v_factory_4',bezeichnung:'Beispiel Freizeit',betrag:0,month:m,year:y,ts:ts+1},{id:'b_factory_3',direction:'expense',catId:'v_factory_7',bezeichnung:'Beispiel Lebensmittel',betrag:0,month:m,year:y,ts:ts+2}];}
  function factoryCredit(){const date=currentDate(),y=date.getFullYear(),m=date.getMonth();return [{id:'k_factory_1',type:'installment',n:'Beispiel Kredit',s:0,r:0,m:0,z:0,g:0,b:'',balanceMonth:m,balanceYear:y}];}
  ...
  function applyFactoryState(){const s=state(),date=currentDate();s.data={};s.cats=[...factoryFixedPositions(),...factoryVariableCategories()];s.kredite=factoryCredit();s.creditMovements=[];s.buchungen=factoryBookings();s.budgets={};s.recurringRules=[];s.annualAdjustments=[];s.percentageAdjustments=[];s.amountAdjustments=[];s.oneTimeEntries=[];s.accountBalances={};s.savingsAccounts=[];s.savingsTransfers=[];s.forecastAssets=emptyForecastAssets();s.forecastAssumptions=defaultForecastAssumptions();s.forecastAccounts=[];s.financialEvents=[];s.forecastScenarios=[];s.forecastGoals=[];s.years=years();s.year=date.getFullYear();s.month=date.getMonth();sortCategoriesInPlace();}
  function clearBookings(){const s=state();s.buchungen=[];s.budgets={};save();}
  function deleteAllEntries(){const s=state();s.data={};s.buchungen=[];s.budgets={};s.kredite=[];s.creditMovements=[];s.recurringRules=[];s.annualAdjustments=[];s.percentageAdjustments=[];s.amountAdjustments=[];s.oneTimeEntries=[];s.accountBalances={};s.savingsAccounts=[];s.savingsTransfers=[];s.financialEvents=[];s.forecastScenarios=[];s.forecastGoals=[];s.forecastAssets=emptyForecastAssets();s.forecastAssumptions=defaultForecastAssumptions();s.forecastAccounts=[];s.cats=factoryVariableCategories();sortCategoriesInPlace();save();}
