...
function openPositionDialog(catId=''){
  ...
  openGenSheet(`<div class="sheet-title">${catId?'Position bearbeiten':'Position hinzufügen'}</div>
    ...
    <div class="field"><div class="lbl">Startjahr der jährlichen Erhöhung</div><div class="sw"><select class="sel" id="pos-inc-year">${S.years.map(y=>`<option value="${y}"${Number(pct?.year??S.year)===y?' selected':''}>${y}</option>`).join('')}</select></div></div>
