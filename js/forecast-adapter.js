...
function forecastBaseMonths(startYear,startMonth,endYear,endMonth=11){
  ...
  for(let index=start;index<=end;index++){
    ...
    for(const credit of S.kredite){
      const balance=Math.max(0,Number(creditBalanceAt(credit,year,month))||0);debt+=balance;
      if(balance>0.005){const interest=Math.max(0,Number(creditInterestAt(credit,year,month))||0);creditPayments+=Math.min(Math.max(0,Number(credit.m)||0),balance+interest);}
    }
...
function buildForecastInput(ui,assetBreakdown,assumptions){
  ...
  return {
    ...
    startAssetBreakdown:{...(assetBreakdown||{})},
    annualReturns:{...(assumptions?.annualReturns||{})},
    purchasingPowerInflation:Number(assumptions?.purchasingPowerInflation)||0,
    savingsTarget:assumptions?.savingsTarget||'etf',
  };
}
